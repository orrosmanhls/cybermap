...
// mobile images
import categoryImageMobile from "../assets/analytics/mobile/mob_img_1_category.jpg";
import totalImageMobile from "../assets/analytics/mobile/mob_img_2_total.jpg";
import exitsImageMobile from "../assets/analytics/mobile/mob_img_3_exits.jpg";
import lifespanImageMobile from "../assets/analytics/mobile/mob_img_4_lifespan.jpg";
...
export default function Analytics(prop) {
  ...
  const isImageLoaded = (imageName) =>
    loadedImages.some((name) => name === imageName);

  const onImageLoad = (imageName) => {
    !isImageLoaded(imageName) &&
      setLoadedImages((prevLoadedImages) =>
        prevLoadedImages.concat([imageName])
      );
  };
  return (
    <div className="careers-view background-dark-grey flex column align-center">
          ...
          <div className="row">
            <div className="col-12 s12 m12 l8">
              <p className="text-justify">
                ...
                findings of solution categories, funding rounds, exits, and
                more!
              </p>
            </div>
          </div>
      ...
      {assets.map((asset) => (
        <div
          ...
          style={allImagesLoaded ? {} : { display: "none" }}
        >
          <img
            src={getImage(asset)}
            className="analytics-image"
            onLoad={() => {
              onImageLoad(asset.name);
            }}
            alt={asset.name}
          />
        </div>
